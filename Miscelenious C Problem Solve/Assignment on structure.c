...
int main()
{
    ...
    // printing the ages of the students in the class before changing:

    printf("\n\n");
    printf("Before changing the age:\n");
    ...
    int returned_id=return_ID(C1);

    printf("Returned Id:%d\n",returned_id);
    printf("The expected age:");

    for(int i=0; i<3; i++)
    {
        if(C1.student_array[i].student_id==returned_id)
        {
    ...
    // printing the ages of the students in the class after using function:
    printf("After changing the age:\n");
    for (int i = 0; i<3; i++)
    {
        printf("%d\n", C1.student_array[i].age);
    }
...
/// The desired function that takes the Class structure as parameter and return the id of the student who is in EEE
/// department who took the microprocessor lab.
...
int return_ID(Class cl)
{
    for(int i=0;i<3;i++)
    {
        for(int j=0;j<5;j++)
        {
            if((strcmp(cl.student_array[i].department,"EEE")==0)&&(strcmp(cl.student_array[i].courses_taken[j].course_name,"MICRO LAB")==0))
                return cl.student_array[i].student_id;
        }
    }
}
